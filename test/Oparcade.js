...
describe("Oparcade", () => {
  ...
  //     // set gid, distributable amount and nonce
  //     gid = 0;
  //     const distributableAmount = 5000;
  //     const nonce = 0;
  //     const feeAmount = (distributableAmount * platformFee) / 1000;

  //     // get signature
  //     let signature = await getSignature(maintainer, gid, alice.address, mockUSDT.address, distributableAmount, nonce);

  //     // claim tokens
  //     await oparcade.connect(alice).claim(gid, alice.address, mockUSDT.address, distributableAmount, nonce, signature);

  //     // check balace
  //     expect(await mockUSDT.balanceOf(feeRecipient.address)).to.equal(feeAmount);
  //     expect(await mockUSDT.balanceOf(alice.address)).to.equal(distributableAmount - feeAmount);
  //   });
  ...
  //     await expect(
  //       oparcade.connect(alice).claim(gid, alice.address, mockUSDT.address, distributableAmount, nonce, signature),
  ...
  //     // claim tokens
  //     await expect(
  //       oparcade.connect(bob).claim(gid, alice.address, mockUSDT.address, distributableAmount, nonce, signature),
  ...
  //     // get signature
  //     let signature = await getSignature(alice, gid, alice.address, mockUSDT.address, distributableAmount, nonce);
